####
#
# This environment differs from MARL_env by its self.cov attribute, 
# where nu and kappa attributes are fixed instead of learned.
#
# You can set the nu and kappa attributes in the GraphMaternKernel_numpy_efficient class
# depending on the covariance matrix you want to use.
#
###
...
class MARL_env_fake_cov(gym.Env):
    ...
    def sample_emissions(self, prev_emissions, actions):
        ...
        if not success:
            ...
            print(final_cov_matrix_t)
            print(np.all(np.linalg.eigvals(final_cov_matrix_t) > 0))
