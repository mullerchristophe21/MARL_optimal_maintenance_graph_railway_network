...
class MARL_env(gym.Env):
    ...
    def save_args_model(self):
        ...
        self.args_model["lambda_act_d1"] = np.concatenate([[1], self.args_model["lambda_act_d1"]])
        self.args_model["lambda_act_d2"] = np.concatenate([[1], self.args_model["lambda_act_d2"]])

        self.args_model["lambda_ind"] = np.concatenate([[1], self.args_model["lambda_ind"]])
        ...
        self.cov = GraphMaternKernel_numpy_efficient(self.eigenpairs, vertex_dim=2, point_kernel=cov_act_ind,
                                                        nu=0.1, kappa=self.args_model["kappa_g"], 
