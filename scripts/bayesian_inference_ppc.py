...
with pm.Model() as m:
    ...
    for t in range(1, n_timesteps):
        ...
        input_gp = pt.stack([
            train_ids.tolist() * 2, 
            stacked_actions_ts[:,t-1],
            [0] * train_ids.shape[0] + [1] * train_ids.shape[0]], axis=1)
...
ppc_mu = pm.sample_posterior_predictive(trace, model=m, var_names=[f"mu_{t}" for t in range(1, n_timesteps)])
az.to_netcdf(ppc_mu, f"./storage_inference/{trace_name}_PPC_mu.nc")

ppc = pm.sample_posterior_predictive(trace, model=m)
az.to_netcdf(ppc, f"./storage_inference/{trace_name}_PPC.nc")
